$(function() {
    ...
    var scrollableSidebar = function() {
        ...
        $('#sidebar.sidebar-fixed .nav').slimScroll({height: (winHeight - 106) + 'px', position: 'left'});
    }
    scrollableSidebar();
    //Submenu dropdown
        ...
        switch (action) {
            case 'collapse':
                ...
                $(this).parents('.box').children('.box-content').slideToggle(500, function(){
                    var parentsTitle = $(this).parents('.box').children('h3').text();
                    if($(this).is(":hidden")) {
                        $(btn).attr('title', 'Expand').children('i').attr('class', 'fa fa-chevron-down');
                    } else {
                        $(btn).attr('title', 'Collapse').children('i').attr('class', 'fa fa-chevron-up');
                    }
                });
                break;
            case 'close':
    ...
    if (jQuery().wysihtml5) {
        $('.wysihtml5').wysihtml5({
            "stylesheets": ["/css/wysiwyg-color.css", "/css/github.css"]
        });
    }
